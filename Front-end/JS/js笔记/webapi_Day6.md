...
^a  以a开头           a$  以a结尾
>如果 ^ 和 $ 在一起，表示必须是精确匹配，里面包含了什么就必须是什么
...
<body>
  <script>
    // 元字符之范围  []  
    // 1. [abc] 匹配包含的单个字符，满足里面的1个就行 多选1
    const reg1 = /^[abc]$/
    console.log(reg1.test('a'))  // true
    console.log(reg1.test('b'))  // true
    console.log(reg1.test('c'))  // true
    console.log(reg1.test('d'))  // false
    console.log(reg1.test('ab'))  // false   精确匹配，只能写一个，因为是a或b或c
...
<body>
  <script>
      console.log(/^java$/i.test('java'))   //加在斜杠/后面
  </script>
</body>
~~~
...
<body>
  <script>
    ...
    // 1. 替换  replace  需求：把前端替换为 web
    // 1.1 replace 返回值是替换完毕的字符串
    //要查找的字符串.replace（按正则表达式找结果，用什么替换）
    // const strEnd = str.replace(/前端/, 'web') 只能替换一个
